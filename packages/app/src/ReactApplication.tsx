...
export function ReactApplication({
  ...
  const viewComponents = React.useMemo(() => {
    const components = resolvedViews
      .map((view) => {
        ...
        return {
          [contextPath + view.id]: ViewComponent,
        };
      })
      .reduce((acc, curr) => ({ ...acc, ...curr }), {});

    if (home) {
      components[contextPath] = components[contextPath + home];
    }
    return components;
  }, [resolvedViews, home]);
  ...
  const providerDefaults = React.useMemo(() => {
    return deepMerge<Partial<import("@pkvsinha/react-hooks").AppContext>>(
      flatDefaults,
  ...
  React.useEffect(() => {
    const initialRuntime = deepMerge<
      import("@pkvsinha/react-hooks").AppContext
    >({} as any, providerDefaults as any, flatFromApp as any);
