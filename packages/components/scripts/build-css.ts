...
const builder = createThemeBuilder().use(basePlugin()).use(layoutPlugin()).use(componentsPlugin());
const css = builder.toString({ minify: false });
const cssMin = builder.toString({ minify: true });


writeFileSync(resolve(outDir, 'pkv.components.css'), css, 'utf8');
writeFileSync(resolve(outDir, 'pkv.components.min.css'), cssMin, 'utf8');